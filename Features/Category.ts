...
// Get Category action
export const getCategoryAction= createAsyncThunk("getCategoryAction",async(data)=>{
    try{
...
// Add Category action
export const addCategoryAction= createAsyncThunk("addCategoryAction",async(data)=>{
    console.log("data Category",data)
    try{
        const response= await axios.post("http://localhost:3000/api/addCategory",data,{
            method:"POST",
            headers:{
...
export const deleteCategoryAction= createAsyncThunk("deleteCategoryAction",async(id,data)=>{
    try{
        const response= await axios.delete(`http://localhost:3000/api/deleteCategory?id=${id}`)
        return response.data
    }catch(error){
...
export const updateCategoryAction=createAsyncThunk("updateCategoryAction",async(data)=>{
    const{id,...updatedCategory}:any=data
    try{
        const response= await axios.put(`http://localhost:3000/api/updateCategory?id=${id}`,updatedCategory,{
            method:"PUT",
            headers:{
                "Content-Type":"application/json"
            }
...
const categorySlice=createSlice({
  ...
  extraReducers:(builder)=>{
    ...
    .addCase(updateCategoryAction.fulfilled,(state,action)=>{
        ...
        const index=state.categories.findIndex((data)=>data._id === updata._id)
